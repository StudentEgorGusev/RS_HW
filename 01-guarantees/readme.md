...
Вы также можете запустить тесты, скомпилировав их локально с помощью компилятора Rust. Такой вариант может быть удобен, если вы хотите лучше изучить или доработать тесты. Убедитесь, что на вашей машине установлен [Rust](https://www.rust-lang.org/tools/install). Скомпилируйте тесты с помощью команды `cargo install --path tests`. Для запуска тестов выполните команду:
...
Код тестов открыт и находится в `tests/src`. Вы можете обращаться к нему и использовать информацию об условиях тестирования, например минимальной и максимальной задержках в сети, в своем решении. Главное, чтобы гарантии обеспечивались всегда, вне зависимости от условий запуска, например вероятности потери сообщений в сети.
...
- Краткий отчёт с описанием вашего решения в файле `solution/readme.md` - обязательно, без него проверка производиться не будет.
...
См. [здесь](https://github.com/systems-group/anysystem/blob/399001f136545adf57f3b65e7de2e36ee4e9e34f/src/python/mod.rs#L229). Данной функции передается объект Sender или Receiver. Измеряется потребление памяти структурами данных (атрибутами) внутри этих объектов. Функция вызывается периодически по ходу выполнения теста, и запоминается максимальное полученное значение, которое выводится в конце теста. Затраты на глобальные переменные и таймеры не учитываются.
...
Если вы претендуете на баллы за оптимизацию потребляемых ресурсов, то нельзя:
- использовать глобальные переменные (кроме иммутабельных данных типа констант),
- хранить полный набор id сообщений в виде битовых значений в числе или bitset-е (тесты проверяют наличие в коде операторов битового сдвига),
